...
-- PAL B&W Zone Test Pattern Generator v2 (no BRAM required)
--
...
--   sel = 4 : HORIZONTAL gradient, black end - same as sel=2 but last zone = black
--   sel = 5 : VERTICAL gradient,   black end - same as sel=3 but last zone = black
--
...
--
-- GRAD_TABLE_B (sel 4/5): identical to GRAD_TABLE but last zone (index 9)
--   is LEVEL_BLANK (black) instead of white.
--
...
entity pal_bw_zonebar_top_v2 is
  generic (
    H_FRONT    : integer := 16;
    H_SYNC_W   : integer := 47;
    H_BACK     : integer := 57;
  ...
  port (
    ...
    -- Pattern select (U8): 0=Vbars 1=Hbars 2=Hgradient 3=Vgradient 4=Hgradient-Bend 5=Vgradient-Bend
...
end entity pal_bw_zonebar_top_v2;

architecture rtl of pal_bw_zonebar_top_v2 is
  ...
  -- Same staircase but last zone (index 9) returns to black (sel 4/5)
  constant GRAD_TABLE_B : grad_table_t := (
    0 => "0100", 1 => "0101", 2 => "0110", 3 => "1000", 4 => "1001",
    5 => "1010", 6 => "1011", 7 => "1101", 8 => "1110", 9 => "0100"
  );
  -- ---------------------------------------------------------
...
begin
  ...
  active_level <= GRAD_TABLE(gzx_idx)   when sel_i = 2 else
                  ...
                  GRAD_TABLE_B(gzx_idx) when sel_i = 4 else
                  GRAD_TABLE_B(gzy_idx) when sel_i = 5 else
