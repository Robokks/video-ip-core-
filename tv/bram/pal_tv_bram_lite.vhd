...
-- PAL B&W BRAM Video IP  --  Lite  (V1.0)
-- ==========================================
--
-- Stripped-down interlaced-only core with 5 pattern selections
-- and a single BRAM frame buffer (no double-buffer swap).
--
-- sel 0  Vertical bars
...
--   tv/bram/pal_tv_bram_lite.vhd  (this file)

entity pal_tv_bram_lite is
  generic (
    H_FRONT    : integer := 16;
    H_SYNC_W   : integer := 47;
    H_BACK     : integer := 57;
  ...
  port (
    clk          : in  std_logic;
    rst          : in  std_logic;
    -- Pattern select: 0=V-bars 1=H-bars 2=H-grad 3=V-grad 4=BRAM
...
end entity pal_tv_bram_lite;

architecture rtl of pal_tv_bram_lite is
...
begin
  ...
  -- -------------------------------------------------------------------------
  -- Active-level mux  (sel 0..4 only; anything else -> black)
  -- -------------------------------------------------------------------------
