...
-- PAL B&W BRAM Video IP  --  Lite v3
-- =====================================
--
-- Extends Lite v2 with full runtime port control of all timing signals.
-- CRIO/host software writes to ports at runtime — no FPGA recompile needed.
--
-- Runtime-controllable ports (0 = use generic default):
--   Active video / Composite blank : v_act_s_f1_p .. v_act_e_f2_p
--   FSS (Field Sync Signal)        : fss_f1_s_p  .. fss_f2_eh_p
--   Frame sync                     : frame_sync_e_p
--   Line sync (H-sync)             : ls_front_p, ls_width_p
--   Vsync window                   : vsync_f1_e_p, vsync_f2_s_p, vsync_f2_e_p
--
-- sel 0  Vertical bars
...
--   tv/bram/pal_tv_bram_lite_v3.vhd  (this file)

entity pal_tv_bram_lite_v3 is
  generic (
    H_FRONT    : integer := 16;
    H_SYNC_W   : integer := 47;
    H_BACK     : integer := 57;
    ...
    FSS_F1_S      : integer := 3;
    FSS_F1_E      : integer := 7;
    FSS_F2_SV     : integer := 315;
    FSS_F2_SH     : integer := 320;
    FSS_F2_EV     : integer := 320;
    FSS_F2_EH     : integer := 320;
    FRAME_SYNC_E  : integer := 7;
    LS_FRONT      : integer := 16;
    LS_WIDTH      : integer := 47;
    VSYNC_F1_E    : integer := 7;
    VSYNC_F2_S    : integer := 312;
    VSYNC_F2_E    : integer := 319
  );
  port (
    clk          : in  std_logic;
    rst          : in  std_logic;
    ...
    -- -----------------------------------------------------------------------
    -- Runtime timing control — CRIO writes these at runtime; 0 = generic default
    -- -----------------------------------------------------------------------
    -- Active video / Composite blank window (10-bit = v_cnt 0-624)
    ...
    -- Frame sync end line (25 Hz pulse HIGH while v_cnt <= this value)
    frame_sync_e_p : in  std_logic_vector(9 downto 0) := (others => '0');
    -- Line sync: front porch start and pulse width (10-bit = h_cnt 0-639)
    ls_front_p    : in  std_logic_vector(9 downto 0) := (others => '0');
    ls_width_p    : in  std_logic_vector(9 downto 0) := (others => '0');
    -- Vsync window: F1 end line, F2 start line, F2 end line
    vsync_f1_e_p  : in  std_logic_vector(9 downto 0) := (others => '0');
    vsync_f2_s_p  : in  std_logic_vector(9 downto 0) := (others => '0');
    vsync_f2_e_p  : in  std_logic_vector(9 downto 0) := (others => '0');
    -- -----------------------------------------------------------------------
    -- Video outputs
    -- -----------------------------------------------------------------------
...
end entity pal_tv_bram_lite_v3;

architecture rtl of pal_tv_bram_lite_v3 is

  constant H_ACT_S    : integer := H_FRONT + H_SYNC_W + H_BACK;  -- 120
  constant V_ACTIVE_F : integer := 288;

  -- -------------------------------------------------------------------------
  -- Runtime integer signals (port value if non-zero, else generic default)
  -- -------------------------------------------------------------------------
  ...
  signal frame_sync_e_i: integer range 0 to 624 := FRAME_SYNC_E;
  signal ls_front_i    : integer range 0 to 639 := LS_FRONT;
  signal ls_width_i    : integer range 1 to 639 := LS_WIDTH;
  signal vsync_f1_e_i  : integer range 0 to 624 := VSYNC_F1_E;
  signal vsync_f2_s_i  : integer range 0 to 624 := VSYNC_F2_S;
  signal vsync_f2_e_i  : integer range 0 to 624 := VSYNC_F2_E;

  -- Zone / gradient constants
  constant NUM_ZONES  : integer := 8;
  constant ZONE_W     : integer := H_ACTIVE  / NUM_ZONES;
  constant ZONE_H     : integer := V_ACTIVE_F / NUM_ZONES;
  constant GZONES     : integer := 10;
  constant GZONE_W    : integer := H_ACTIVE  / GZONES;
  constant GZONE_H    : integer := V_ACTIVE_F / GZONES;
  constant GZONES_B   : integer := 11;
  constant GZONE_W_B  : integer := H_ACTIVE  / GZONES_B;
  constant GZONE_H_B  : integer := V_ACTIVE_F / GZONES_B;
...
begin
  ...
  -- -------------------------------------------------------------------------
  -- Runtime port resolver: 0 = use generic default, non-zero = override now
  -- -------------------------------------------------------------------------
  ...
  frame_sync_e_i<= to_integer(unsigned(frame_sync_e_p))when unsigned(frame_sync_e_p)/= 0 else FRAME_SYNC_E;
  ls_front_i    <= to_integer(unsigned(ls_front_p))    when unsigned(ls_front_p)    /= 0 else LS_FRONT;
  ls_width_i    <= to_integer(unsigned(ls_width_p))    when unsigned(ls_width_p)    /= 0 else LS_WIDTH;
  vsync_f1_e_i  <= to_integer(unsigned(vsync_f1_e_p))  when unsigned(vsync_f1_e_p)  /= 0 else VSYNC_F1_E;
  vsync_f2_s_i  <= to_integer(unsigned(vsync_f2_s_p))  when unsigned(vsync_f2_s_p)  /= 0 else VSYNC_F2_S;
  vsync_f2_e_i  <= to_integer(unsigned(vsync_f2_e_p))  when unsigned(vsync_f2_e_p)  /= 0 else VSYNC_F2_E;
  ...
  -- -------------------------------------------------------------------------
  -- PAL interlaced composite sync (csync + field only; active computed below)
  -- -------------------------------------------------------------------------
  ...
  -- -------------------------------------------------------------------------
  -- Active region (runtime ports control composite blank boundaries)
  -- -------------------------------------------------------------------------
  ...
  -- -------------------------------------------------------------------------
  -- Sync outputs (all runtime-controllable)
  -- -------------------------------------------------------------------------
  -- Vsync window: runtime ports control F1 end and F2 start/end lines
  in_vsync_s <= '1' when (v_cnt <= vsync_f1_e_i) or
                          (v_cnt >= vsync_f2_s_i and v_cnt <= vsync_f2_e_i)
                else '0';

  -- Line sync: runtime ports control front porch position and pulse width
  line_sync_s  <= '1' when h_cnt >= ls_front_i and
                            h_cnt < ls_front_i + ls_width_i and
                            in_vsync_s = '0'
                  else '0';

  -- Frame sync: runtime port controls end line of 25 Hz pulse
  frame_sync_s <= '1' when v_cnt <= frame_sync_e_i else '0';

  -- FSS: runtime ports control all F1 and F2 boundaries
